...
def plot_fss_chi(SIZES, ax, beta_c, nu, gamma):
...
Use --plot-fss-fit as a call option to plot finite-size scaled data of magnetic susceptibility using critial parameters obtained from fit analysis."
	...
	if len(sys.argv) != 2:
		...
		if user_mode == "--plot":	
			...
			# Initialize plot
			print("Plotting magnetic susceptibility...")
			fig, ax = plt.subplots(1, 1, figsize=(4,3))
			plot_chi(SIZES, ax)
			plt.savefig(repo.working_tree_dir + "/analysis/magnetic-susceptibility/results/magnetic_susceptibility_plot.pdf")
			plt.show() # TODO Comment
			plt.close()
			print("Done! Check the pdf file at " + repo.working_tree_dir + "/analysis/results/magnetic-susceptibility/")
			...
			# Initialize plot
			print("Plotting FSS magnetic susceptibility using theoretical critical parameters...")
			fig, ax = plt.subplots(1, 1, figsize=(4,3))
			plot_fss_chi(SIZES, ax, beta_c, nu, gamma)
			plt.savefig(repo.working_tree_dir + "/analysis/magnetic-susceptibility/magnetic_susceptibility_plot_FSS_th.pdf")
			# plt.show()
			plt.close()
			print("Done! Check the pdf file at " + repo.working_tree_dir + "/analysis/magnetic-susceptibility/results/")
			...
			# Initialize plot
			print("Plotting FSS magnetic susceptibility using fitted critical parameters...")
			fig, ax = plt.subplots(1, 1, figsize=(4,3))
			plot_fss_chi(SIZES, ax, beta_c, nu, gamma)
			plt.savefig(repo.working_tree_dir + "/analysis/magnetic-susceptibility/results/magnetic_susceptibility_plot_FSS_fit.pdf")
			plt.close()
			print("Done! Check the pdf file at " + repo.working_tree_dir + "/analysis/magnetic-susceptibility/results/")
